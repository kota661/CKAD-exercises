...
### コマンド「ls」を実行するだけの活性プローブを持つnginxポッドを作成します。 YAMLをpod.yamlに保存します。 実行して、プローブのステータスを確認し、削除します。
...
### pod.yamlファイルを変更して、活性プローブが5秒後に開始し、プローブ間の間隔が5秒になるようにします。 それを実行し、プローブをチェックして削除します。
...
### ポート80のパス '/'にHTTP readinessProbeを使用してnginxポッド（ポート80を含む）を作成します。もう一度、それを実行し、readinessProbeを確認して削除します。
...
### busyboxのPodを作成し'i=0; while true; do echo "$i: $(date)"; i=$((i+1)); sleep 1; done'を実行。ログを確認する
...
### 'ls /notexist'を実行するbusyboxポッドを作成します。 エラーがあるかどうかを判断します（もちろんあります）。 最後に、ポッドを削除します
...
### 'notexist'を実行するbusyboxポッドを作成します。 エラーがあるかどうかを判断します（もちろんあります）。 最後に、猶予期間0でポッドを強制的に削除します
...
### ノードのCPU /メモリ使用率を取得します（[metrics-server]（https://github.com/kubernetes-incubator/metrics-server）が実行されている必要があります）
